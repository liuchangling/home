...
export default {
  ...
  watch: {
    ...
    'rangeState.startDate'(newVal) {
      this.markRange(this.rangeState.startDate, this.rangeState.endDate)
    },
  ...
  methods: {
    ...
    handleMouseMove(event) {
      ...
      if (this.selectionMode === 'weekrange') {
        const hoverDate = this.getDateOfCell(row, column)
        if (!this.rangeState.selecting) {
          this.$emit('changerange', {
            ...
            rangeState: {
              selecting: false,
              startDate: this.getMonday(hoverDate),
              endDate: this.getSunday(hoverDate)
            }
          })
        } else if (hoverDate >= this.minDate) {
          this.$emit('changerange', {
            minDate: this.minDate,
            maxDate: this.maxDate,
            rangeState: {
          ...
          this.$emit('changerange', {
            ...
            rangeState: {
              selecting: true,
              startDate: this.getMonday(hoverDate),
              endDate: this.getSunday(this.minDate)
            }
          })
        }
      }
    ...
    getMonday(date) {
      // 国际以周日作为第一天修正
      return dayjs(date).subtract(1, 'day').startOf('week').add(1, 'day').toDate()
    },
    // 获取周日
    getSunday(date) {
      // 国际以周日为第一天修正
      return dayjs(date).subtract(1, 'day').endOf('week').add(1, 'day').toDate()
    },
    ...
    handleClick(event) {
      ...
      if (this.selectionMode === 'range') {
        ...
        if (!this.rangeState.selecting) {
          this.$emit('pick', { minDate: this.getMonday(newDate), maxDate: null })
          this.rangeState.selecting = true
          this.$nextTick(() => {
            this.$emit('changerange', {
              minDate: this.minDate,
              ...
              rangeState: {
                selecting: true,
                startDate: this.getMonday(newDate),
                endDate: this.getSunday(newDate)
              }
            })
          })
        } else {
          if (newDate >= this.minDate) {
            this.$emit('pick', { minDate: this.getMonday(this.minDate), maxDate: this.getSunday(newDate) })
          } else {
            this.$emit('pick', { minDate: this.getMonday(newDate), maxDate: this.getSunday(this.minDate) })
          }
          this.rangeState.selecting = false
        }
      }
